import 'package:flutter/cupertino.dart';

class Styles {
  static const titleLarge30 = TextStyle(
      fontSize: 30, fontWeight: FontWeight.w600, letterSpacing: 0.1, fontFamily: "GT Sectra Fine");

  static const titleMedium20 =
      TextStyle(fontSize: 20, fontWeight: FontWeight.w600, fontFamily: "GT Sectra Fine");

  static const titleMedium18 = TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.w600,
    letterSpacing: 0.1,
  );

  static const titleSmall16 = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w600,
    letterSpacing: 0.1,
  );

  static const titleSmall14 = TextStyle(
    fontSize: 14,
    fontWeight: FontWeight.w500,
    letterSpacing: 0.1,
  );
}
